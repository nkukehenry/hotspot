...
<head>
    ...
    <title>{{ $settings?->system_name ?? config('app.name', 'Neonet') }}</title>
    ...
    <script>
        ...
        (function() {
            ...
            document.addEventListener('DOMContentLoaded', () => {
                ...
                if (!hasSiteData && !isSitesPage && cachedSite) {
...
<body class="bg-gray-200 dark:bg-gray-900"
    style="background-image:url({{ asset('images/bg.webp') }})!important; background-size:cover!important; background-repeat:no-repeat; min-height:100vh; background-position:center;">
    ...
    <div class="container mx-auto px-4 sm:px-6 lg:px-8">
        @if (session('success') && !request()->routeIs('customer.payment'))
            <div class="alert alert-success text-center text-bold w-full mt-5 text-green-600">
                {{ session('success') }}
            </div>
        @endif

        @if (session('error') && !request()->routeIs('customer.payment'))
